...
def DeleteDuplicate(Path="Marvellous"):
    ...
    if os.path.exists("LogFiles") == False:
        os.mkdir("LogFiles")

    timestamp = time.ctime()
    LogFileName = "MarvellousLog_%s.log" % timestamp
    LogFileName = LogFileName.replace(" ", "_").replace(":", "_")
    LogFilePath = os.path.join("LogFiles", LogFileName)

    fobj = open(LogFilePath, "w")

    fobj.write("-" * 60 + "\n")
    fobj.write("Duplicate File Deletion Log\n")
    fobj.write("Time: " + timestamp + "\n")
    fobj.write("Directory Scanned: " + os.path.abspath(Path) + "\n")
    fobj.write("-" * 60 + "\n\n")
